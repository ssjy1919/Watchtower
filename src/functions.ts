 ...
 */
export function fileInfo(): { name:string,path: string, stat: FileStats }[] {
  ...
  // 遍历文件列表，收集所有文件的信息
  const filesInfo = markdownFiles.map(file => ({ name:file.basename,path: file.path, stat: file.stat }));

  // 如果没有找到文件，返回一个空数组
  return filesInfo.length > 0 ? filesInfo : [{ name:'',path: '', stat: { size: 0, ctime: 0, mtime: 0 } }];
}
